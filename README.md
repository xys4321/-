...
<body>
    ...
    {
        "id": 23,
        "question": "在BIM协同设计中，哪个工具可以帮助团队成员实时查看和编辑模型?",
        "options": [
            "A. 工作集",
            "B. 链接模型",
            "C. 云协作平台",
...
{
    id: 87,
    question: "在BIM技术中，哪个环节通常用于进行建筑性能分析?",
    options: [
        "A.建模",
        "B.碰撞检测",
        "C.可视化",
        ...
        function submitAnswers() {
            ...
            if (wrongQuestions.length === 0) {
                ...
                wrongQuestions.forEach(q => {
                    ...
                    wrongDiv.innerHTML = `
                        <p><strong>${q.type === 'single' ? '单选题' : '多选题'} 题号 ${q.id}:</strong> ${q.question}</p>
                        <p>您的答案: <span class="highlight">${q.userAnswer}</span></p>
                        <p class="correct-answer">正确答案: ${q.correctAnswer}</p>
                    `;
                    wrongQuestionsList.appendChild(wrongDiv);
                });
            }
